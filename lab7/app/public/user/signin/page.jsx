...
export default function SignInForm() {
  ...
  const params = useSearchParams(); // Fetch query parameters (returnUrl)
  const router = useRouter(); // Next.js router for redirection
  const returnUrl = params.get("returnUrl") || "/"; // Fallback return URL
  const [error, setError] = useState(null); // State to manage error popup visibility
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setError("Invalid credentials. Please try again."); // Set error for invalid credentials
    }
  };
  ...
  return (
    <div>
      ...
      {error && (
        <ErrorPopup
          message={error}
          onClose={() => setError(null)} // Close popup
        />
      )}
